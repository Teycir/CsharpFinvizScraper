...
{
    public partial class FrmScreener : Form, IViewScreener
    {
        ...
        public FrmScreener()
        {
            ...
            labelCurDateTxt.Text = DateTime.Now.ToShortDateString();
             List<string> dbData =
                    XmlReadWrite.ReadXMLData("DbData");
       ...
       _connectionstring =
        ...
        private void FillComboboxes()
        {
            ...
            _pres.FillDropDownList("select distinct industry from finviz.findata  where trim(industry) <> 'Exchange Traded Fund' order by industry;", comboBoxIndustry,
                                   "industry",
                                   _connectionstring);
        ...
        {
            _heartbeat = new Thread(state => _pres.UpdateFinvizDb(labelUpdateStatusContent, "Data updated"));
        ...
        private void buttonTppOb_Click(object sender, EventArgs e)
        {
            ...
            from finviz.findata where (price > 0.2 and targetprice <> 0 and volume>10000 " +
                        GetWhereQuery() + " and iddownload = (select iddownload from  finviz.findata order by iddownload desc limit 1)"
                        +
            ...
            List<string> values;
            DbConnect.GetValue(query, "ticker", _connectionstring, out values);
            textBoxResultOB.Clear();
        ...
        private void buttonGetRatios_Click(object sender, EventArgs e)
        {
            ...
            string query1;
                ...
                query =
                    @"select distinct concat(ticker,':  ',left((lastinsiderpurchaseprice / price),6),' =>                     ',industry) as ticker
            from finviz.findata where (price > 0.2 and lastinsiderpurchaseprice <> 0 and volume>10000 " +
                      "and ticker in (" + FitForQuery(textBoxTicker.Text.ToUpper()) + " and iddownload = (select iddownload from  finviz.findata order by iddownload desc limit 1)" + ")" +
                    ") order by (lastinsiderpurchaseprice / price) desc;";
            ...
            from finviz.findata where (price > 0.2 and targetprice <> 0 and volume>10000 " +
                    "and ticker in (" + FitForQuery(textBoxTicker.Text.ToUpper()) + " and iddownload = (select iddownload from  finviz.findata order by iddownload desc limit 1)" + ")" +
                    ")  order by (targetprice / price) desc;";
           
            List<string> values1;
            DbConnect.GetValue(query1, "ticker", _connectionstring, out values1);
            textBoxRatios.AppendText("\r\n");
            textBoxRatios.AppendText(" Avg.Analy.Targ.Price/Price" + "\r\n");
